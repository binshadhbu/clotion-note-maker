...
export const getSidebar= query({
    ...
    handler:async(ctx,args)=>{
        ...
        const documents= await ctx.db.query("documents")
            .withIndex("by_user_parent",(q)=>q.eq("userId",userId).eq("parentDocument",args.parentDocument)).filter((q)=>q.eq(q.field("isArchived"),false)).order("desc").collect();
        return documents;
    },
});
...
export const getTrash=query({
    handler:async(ctx)=>{
        ...
        const documents= await ctx.db.query("documents")
            .withIndex("by_user",(q)=>q.eq("userId",userId)).filter((q)=>q.eq(q.field("isArchived"),true)).order("desc").collect();
        return documents;
    }
});
...
export const removeIcon=mutation({
    ...
    handler:async (ctx,args)=>{
        ...
        if(existingDocument.userId!==userId){
            throw new Error("not authorized to remove icon from this document");
        }
...
export const removeCoverImage=mutation({
    ...
    handler:async(ctx,args)=>{
        ...
        if(existingDocument.userId!==userId){
            throw new Error("not authorized to remove cover image from this document");
        }
